...
class RunStats:
    ...
    def to_text(self) -> str:
        return (
            ...
            f"Latency (ms): p50={self.p50_ms:.2f} p95={self.p95_ms:.2f} p99={self.p99_ms:.2f}\n"
            f"Process CPU time (s): user={self.cpu_user_s:.2f} system={self.cpu_system_s:.2f}\n"
...
def capture_process_snapshot(pid: int | None = None) -> tuple[float, float, float]:
    pid = pid or os.getpid()
    p = psutil.Process(pid)
    ct = p.cpu_times()
    rss = p.memory_info().rss / (1024 * 1024)
    return (ct.user, ct.system, rss)
